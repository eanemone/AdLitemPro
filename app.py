...
def create_docx(content: str) -> BytesIO:
    ...
    # --- LOGIC FIX: Map parts correctly ---
    if len(parts) >= 3:
        # Standard Format: Question -> Brief Answer -> Discussion
        sections = [
            ("QUESTION PRESENTED", parts[0]),
            ("BRIEF ANSWER", parts[1]),
            ("DISCUSSION", parts[2])
        ]
    elif len(parts) == 2:
        # Fallback: Question -> Brief Answer (Missing Discussion)
        sections = [
            ("QUESTION PRESENTED", parts[0]),
            ("BRIEF ANSWER", parts[1])
        ]
    ...
    for header, section_text in sections:
        ...
        for line in section_text.split('\n'):
            ...
            # Skip lines that are just duplicated headers like "Discussion"
            if line.upper() == header: continue

            # Clean HTML Citations for Word (Remove spans, keep text)
...
2. **CITATION SENTENCES (THE "FULL STOP" RULE):**
   - **RULE:** A citation MUST be its own standalone sentence. It cannot be attached to the previous sentence.
   - **INSTRUCTION:** Finish your fact sentence with a period. THEN add the citation.
   - ❌ WRONG (Run-on): The worker must sign the form <span class="inline-citation">Source</span>.
   - ✅ RIGHT (Standalone): The worker must sign the form. <span class="inline-citation">Source</span>.
   - **NOTE:** Ensure there is a period BEFORE the citation tag and a period AFTER the citation tag.
